...
type Props = DrawerScreenProps<DrawerParamList, 'MovementsCreate'>;

const MovementsCreateScreen = ({ navigation }: Props) => {

  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [yearFoundation, setYearFoundation] = useState('');
  ...
  const handleSave = async () => {
    setSaving(true);
    const res = await fetch('http://localhost:8000/movimentos/', {
      method: 'POST',
...
export default MovementsCreateScreen;